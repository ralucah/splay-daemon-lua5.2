...
LUALIB_API int luaopen_splay_core(lua_State *L)
{
    luaL_newlib(L, sp_funcs);
...
int sp_exec(lua_State *L)
{
	...
	for (i = 1; i <= num_args; i++) {
		if (lua_type(L, i) == LUA_TSTRING) {
			...
			int tmp_integer = lua_tointegerx(L, i, NULL);
