...
local function peer_receive(id)
	...
	while true do
		...
		local ok, msg = pcall(function() return lbinenc.decode(msg_s) end)
		if not ok then
	...
	while true do
		...
		if #messages[id] > 0 and #messages_sent[id] < settings.window_size then
			...
			if not msg.timeout or
				...
				dup.time=nil
				dup.timeout=nil
				if not s:send(lbinenc.encode(dup)) then break end
				p.last = misc.time()
			end
		end
	end
...
local function rpc_handler(s)
	...
	while true do
		...
		msg.time = nil
		msg.timeout = nil
		if not s:send(lbinenc.encode(msg)) then break end
	end
