...
local function async(sc, handler, s_s, connect)
	events.thread(function()
		...
		if handler.initialize then
			...
			if not ok then splay_net.l_o:warning("async initialize: "..msg) end
		end
		...
		if init then
			...
			if handler.receive then
				t_r = events.thread(function()
					...
					if not ok then splay_net.l_o:warning("async receive: "..msg) end
			...
			if handler.send then
				t_s = events.thread(function()
					...
					if not ok then splay_net.l_o:warning("async send: "..msg) end
					events.fire("net:wait_"..sc_string)
				end)
			end
			...
			splay_net.l_o:notice("async end: "..sc_string)

			if handler.finalize then
				local ok, msg = pcall(function() handler.finalize(sc, connect) end)
				if not ok then splay_net.l_o:warning("async finalize: "..msg) end
			end
		end
...
function splay_net.client(ip, port, handler, timeout)
	...
	if s then
		...
		splay_net.l_o:warning("Cannot create a new socket: "..msg)
	end
end

-- Additionnal socket functions
function splay_net.server(port, handler, max, filter, backlog)
	...
	if filter and type(filter) ~= "function" then
		no_close = filter
		filter = nil
		splay_net.l_o:warn("net.server() option 'no_close' is no more supported, check doc")
	end
	local ip="*"
		...
		while true do
			...
			if sc then
				...
				if filter then
					...
					ok = filter(ip, port)
					if not ok then
						splay_net.l_o:notice("Refused by filter", ip, port)
					end
				end
				...
				if ok then
					...
					if type(handler) == "function" then
						events.thread(function()
							...
							if not ok then splay_net.l_o:warning("handler: "..msg) end
							if _s_s[port] then
				...
				if _s_s[port].stop then
					splay_net.l_o:warn("server on port "..port.." stopped")
					_s_s[port] = nil
					break
				else
					splay_net.l_o:warn("server accept(): "..err)
				end
				events.yield()
			end
		end
...
function splay_net.stop_server(port, kill_clients)
	if type(port) == "table" then
		port = port.port
	end
	if _s_s[port] then
...
-- Only use one socket.
--]]
function splay_net.udp_helper(port, handler)
	if type(port) == "function" then
		local tmp = port
		port = handler
		handler = tmp
