...
core = require"splay.misc_core" -- register splay.misc
...
--[[ String generation ]]--
function splay_misc.gen_string(times, s)
...
function splay_misc.split(s, sep)
       local res = {}
       sep = sep or ' '
...
function splay_misc.table_keyset(t)
	local s = {}
...
-- DEPRECATED
function splay_misc.table_concat(t1, t2)
...
function splay_misc.equals(e1, e2)
...
splay_misc.equal = splay_misc.equals -- for compatibility reasons, keep old version with typo.

function splay_misc.empty(t)
	if next(t) then
		return false
	else
		return true
...
function splay_misc.random_pick(a, n) -- array, number of elements to pick
	local ori_n = n
	n = n or 1
...
function splay_misc.random_pick_one(a)
	return random_pick(a)
end

function splay_misc.shuffle(a)
	if #a == 1 then
		return a
	else
...
function splay_misc.to_string(int, size)
...
function splay_misc.hash_ascii_to_byte(s)
	local hash = ""
	for i = 1, #s, 2 do
...
function splay_misc.dec_to_base(input, b)
...
function splay_misc.base_to_dec(input, b)
...
function splay_misc.convert_base(input, b1, b2)
	b1 = b1 or 10
	b2 = b2 or 16
	return dec_to_base(base_to_dec(input, b1), b2)
end

ctime = core.time -- time from core C lib
--[[ Unix time with precision of 1/10'000s ]]--
function splay_misc.time()
	local s, m = ctime()
	return s + m / 1000000
end
...
-- a: start
-- b: end
function splay_misc.between_c(i, a, b)
...
function splay_misc.to_dec_string(s)
	s = tostring(s)
...
function splay_misc.stringify(arg)
...
function splay_misc.assert_object(object)
	local wrapped = {}
	local mt = {
...
function splay_misc.assert_function(func)
	return function(...)
		return assert(func(...))
	end
end
...
function splay_misc.call(procedure)
...
function splay_misc.throw(name, value)
	error({exception = {name, value}}, 0)
end

function splay_misc.try(f, ecatch)
	local r = {pcall(f)}
	if not r[1] then
