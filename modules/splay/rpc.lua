...
splay_rpc._COPYRIGHT   = "Copyright 2006 - 2011"
splay_rpc._DESCRIPTION = "Remote Procedure Call over TCP"
splay_rpc._VERSION     = 1.0

--[[ DEBUG ]]--
local _NAME = "splay.rpc"
splay_rpc.l_o = log.new(3, "[".._NAME.."]")
...
function splay_rpc.stats()
	return number
end

function splay_rpc.infos()
...
local function reply(s, data)
	...
	if not ok then
		splay_rpc.l_o:warn("reply send(): "..err)
	end
	return ok, err
end
		...
		if ok then
			if data.type == "call" then
				...
				if c then
					...
					splay_rpc.l_o:warn("rpc_handler misc.call(): "..err)
			...
			splay_rpc.l_o:warn("rpc_handler corrupted message:", data_s)
		end
	else
		splay_rpc.l_o:warn("rpc_handler receive(): "..err)
	end
end

function splay_rpc.server(port, max, backlog)
	return net.server(port, rpc_handler, max, nil, backlog)
end

function splay_rpc.stop_server(port)
		...
		if r then
			...
			if timeleft then
				...
				if timeleft <= 0 then
					...
					splay_rpc.l_o:warn(err_prefix.." before send timeout")
			...
			if not r then
				s:close()
				if call_s then call_s:unlock() end
				splay_rpc.l_o:warn(err_prefix.." send(): "..err)
				return false, err
			end
			...
			if r then
				if data.type == "call" then
					...
					if ok then
						...
						splay_rpc.l_o:warn("corrupted message")
				...
				splay_rpc.l_o:warn(err_prefix.." receive(): "..err)
				return false, err
			end
		else
			s:close()
			if call_s then call_s:unlock() end
			splay_rpc.l_o:warn(err_prefix.." connect("..ip..":"..port.."): "..err)
			return false, err
		end
	else
		if call_s then call_s:unlock() end
		splay_rpc.l_o:error(err_prefix.." tcp(): "..err)
		return false, err
	end
end
...
-- return: true|false, array of responses
function splay_rpc.acall(ip, port, call, timeout)

	-- support for a node array with ip and port
	if type(ip) == "table" then
		if not ip.ip or not ip.port then
			splay_rpc.l_o:warn("parameter array without ip or port")
	...
	if timeout ~=nil and tonumber(timeout)==nil then
		 splay_rpc.l_o:warn("invalid timeout value: ",timeout)
...
-- DEPRECATED
function splay_rpc.a_call(...) return acall(...) end

function splay_rpc.ecall(ip, port, func, timeout)
...
function splay_rpc.call(ip, port, func, timeout)
	local ok, r = acall(ip, port, func, timeout)
	if ok then
		return unpack(r)
	else
...
-- RPC ping
function splay_rpc.ping(ip, port, timeout)
	-- support for a node array with ip and port
...
function splay_rpc.proxy(ip, port)
	local p = {}
	if type(ip) == "table" then
		p.port = ip.port
		p.ip = ip.ip
