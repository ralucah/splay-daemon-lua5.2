...
splay_urpc._COPYRIGHT   = "Copyright 2006 - 2011"
splay_urpc._DESCRIPTION = "UDP RPC"
splay_urpc._VERSION     = 1.0

--[[ DEBUG ]]--
local _NAME = "splay.urpc"
splay_urpc.l_o = log.new(3, "[".._NAME.."]")
...
local base_key = nil
local c = 0
function splay_urpc.get_key(seed)
...
function splay_urpc.stats()
	return number
end

function splay_urpc.infos()
...
local function clean_replied()
	...
	for key, d in pairs(replied) do
		...
		if settings.cleaning_after and d.time + settings.cleaning_after < now then
			--splay_urpc.l_o:debug("CLEANING", key)
			replied[key] = nil
		end
	end
end
...
local function reply(s, data, ip, port)
	...
	if data.type == "call" then
		if not replied[data.key] then
			--splay_urpc.l_o:debug("call()", data.key)
		...
		if length > 8192 then
			...
			reply_s = enc.encode(data)
			splay_urpc.l_o:warning("reply(): too much data")
		end
	else
		reply_s = enc.encode(data)
	end
	...
	if not ok then
		splay_urpc.l_o:warn("sendto(): "..err)
	end
end
...
local function process_one_msg(s, data, ip, port)
	...
	if ok then
		...
		splay_urpc.l_o:warn("corrupted message")
	end
end
...
local function sender(s)
	while true do
		...
		--splay_urpc.l_o:debug("sender() loop")
		local q = {}
		...
		for key, data in pairs(messages) do
			if data.next_try <= now then
				...
				--splay_urpc.l_o:debug("try", data.nb_try, data.key)
		...
		if next_wakeup then
			--splay_urpc.l_o:debug("wait", next_wakeup - now)
			events.wait("urpc:sender", next_wakeup - now)
		else
			--splay_urpc.l_o:debug("wait")
...
local function receiver(s)
	while true do
		...
		if data then
			...
			if ip == "timeout" then
				splay_urpc.l_o:warn("receivefrom(): "..ip)
			else
				splay_urpc.l_o:notice("receivefrom(): server closed")
				break
			end
		end
	end
...
function splay_urpc.server(port)
	...
	if not s then
		splay_urpc.l_o:warn("udp():"..err)
		return nil, err
	end

	splay_urpc.l_o:notice("URPC server bound on port "..port)
	
	local r, err = s:setsockname(ip, port)
	
	if not r then
		splay_urpc.l_o:warn("setsockname("..port.."): "..err)
		return nil, err
	end
...
function splay_urpc.stop_server(port)
...
function splay_urpc.default_server()
...
local function do_call(ip, port, typ, call, timeout)
	...
	if (timeout and settings.cleaning_after and
		...
		splay_urpc.l_o:warn("do_call adjusted timeout", timeout)
	...
	local l = #edatac
	if l > 8192 then
		splay_urpc.l_o:warn("RPC UDP too big to be sent: "..l)
...
-- return: true|false, array of responses
function splay_urpc.acall(ip, port, call, timeout)

	-- support for a node array with ip and port
	if type(ip) == "table" then
		if not ip.ip or not ip.port then
			splay_urpc.l_o:warn("parameter array without ip or port")
	...
	if timeout ~=nil and tonumber(timeout)==nil then
		splay_urpc.l_o:warn("invalid timeout value: ",timeout)
...
-- DEPRECATED
function splay_urpc.a_call(...) return acall(...) end
...
function splay_urpc.call(ip, port, func, timeout)
	local ok, r = acall(ip, port, func, timeout)
	if ok then
		return unpack(r)
	else
...
-- RPC ping
function splay_urpc.ping(ip, port, timeout)
	-- support for a node array with ip and port
...
function splay_urpc.proxy(ip, port)
	local p = {}
	if type(ip) == "table" then
		p.port = ip.port
		p.ip = ip.ip
