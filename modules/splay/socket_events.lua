...
local socket_core = require"socket" -- needed for dns to ip
...
--[[ DEBUG ]]--
local _NAME="splay_socket_events"
splay_socket_events.l_o = log.new(3, "[".._NAME.."]")
...
local function receive(socket, pattern, part, timeout)
	--splay_socket_events.l_o:debug("receive("..tostring(pattern)..")", timeout)
			...
			if err ~= "timeout" then
				...
				if end_time then
					...
					if end_time > ct then
						...
						splay_socket_events.l_o:notice("receive() timeout ("..timeout..")")
						return data, err, part
					end
				else
...
local function send(socket, data, i, j, timeout)
	--splay_socket_events.l_o:debug("send("..string.sub(data, 1, 20).."...("..string.len(data).."))", timeout)
			...
			if err ~= "timeout" then
				...
				if end_time then
					...
					if end_time > ct then
						...
						splay_socket_events.l_o:notice("send() timeout ("..timeout..")")
						return n, err, last
					end
				else
		...
		if err == "timeout" or err == "timedout" or err == "Operation already in progress" then
			if end_time then
				...
				if end_time > ct then
					...
					splay_socket_events.l_o:notice("accept() timeout ("..timeout..")")
					return nil, "timeout"
				end
			else
				coroutine.yield("event:receive", socket)
...
local function connect(socket, ip, port, timeout)
	--splay_socket_events.l_o:debug("connect("..ip..", "..port..", "..tostring(timeout)..")")
		...
		if err == "timeout" or
			...
			if end_time then
				...
				if end_time > ct then
					...
					splay_socket_events.l_o:notice("connect() timeout ("..timeout..")")
					return nil, "timeout"
				end
			else
				coroutine.yield("event:send", socket)
...
local function udp_receive(socket, from, size, timeout)
	--splay_socket_events.l_o:debug("udp_receive("..tostring(timeout)..")")
...
function splay_socket_events.wrap_tcp(socket)
	splay_socket_events.l_o:debug("wrap_tcp("..tostring(socket)..")")
	...
	if socket.settimeout then
		wrapped_socket.settimeout = function(self, to)
			...
			--splay_socket_events.l_o:debug("settimeout("..tostring(to)..")")
			timeout = to
...
local function wrap_udp(socket)
	splay_socket_events.l_o:debug("wrap_udp("..tostring(socket)..")")
...
function splay_socket_events.wrap(socket, err)
	splay_socket_events.l_o:debug("wrap("..tostring(socket)..")")
	if string.find(tostring(socket), "#SE") then
		splay_socket_events.l_o:warn("trying to wrap an already SE socket "..tostring(socket))
		return socket
	end

	if not socket.tcp then
		splay_socket_events.l_o:error("Non socket object: "..tostring(socket))
