...
LIBS= -L/usr/lib -L/usr/lib/x86_64-linux-gnu -llua5.2 -lm splay_core.so misc_core.so data_bits_core.so
...
all: splay_core.so misc_core.so data_bits_core.so splayd jobd luacrypto/crypto.so cert lbase64/base64.so
...
lbase64/base64.so: lbase64/lbase64.o
	$(CC) -O -fpic -shared -o lbase64/base64.so lbase64/lbase64.o
